...
func authorizedInsertWithReturn[ObjectType any, ArgumentType any,
	...
	return func(ctx context.Context, arg ArgumentType) (empty ObjectType, err error) {
		...
		err = authorizer.ByRoleName(ctx, act.SubjectID, rbac.RoleNames(act.Roles.Names()), act.Scope, act.Groups, action, object.RBACObject())
		if err != nil {
...
func authorizedFetchSet[ArgumentType any, ObjectType rbac.Objecter,
	...
	return func(ctx context.Context, arg ArgumentType) (empty []ObjectType, err error) {
		...
		// Authorize the action
		return rbac.Filter(ctx, authorizer, act.SubjectID, rbac.RoleNames(act.Roles.Names()), act.Scope, act.Groups, rbac.ActionRead, objects)
	}
}
...
func authorizedQueryWithRelated[ObjectType any, ArgumentType any, Related rbac.Objecter](
	...
	return func(ctx context.Context, arg ArgumentType) (empty ObjectType, err error) {
		...
		// Authorize the action
		err = authorizer.ByRoleName(ctx, act.SubjectID, rbac.RoleNames(act.Roles.Names()), act.Scope, act.Groups, action, rel.RBACObject())
		if err != nil {
			return empty, xerrors.Errorf("unauthorized: %w", err)
		}
...
func prepareSQLFilter(ctx context.Context, authorizer rbac.Authorizer, action rbac.Action, resourceType string) (rbac.PreparedAuthorized, error) {
	...
	return authorizer.PrepareByRoleName(ctx, act.SubjectID, rbac.RoleNames(act.Roles.Names()), act.Scope, act.Groups, action, resourceType)
}
